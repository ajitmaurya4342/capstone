...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ErrorResponse> bad(IllegalArgumentException e, HttpServletRequest r) {
        return ResponseEntity.badRequest()
                .body(new ErrorResponse(Instant.now(), r.getRequestURI(), "VALIDATION_ERROR", e.getMessage()));
    }

    @ExceptionHandler(NoSuchElementException.class)
    ResponseEntity<ErrorResponse> notFound(NoSuchElementException e, HttpServletRequest r) {
        return ResponseEntity.status(404)
                .body(new ErrorResponse(Instant.now(), r.getRequestURI(), "NOT_FOUND", e.getMessage()));
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    ResponseEntity<ErrorResponse> validation(MethodArgumentNotValidException e, HttpServletRequest r) {
        return ResponseEntity.badRequest()
                .body(new ErrorResponse(Instant.now(), r.getRequestURI(), "VALIDATION_ERROR",
